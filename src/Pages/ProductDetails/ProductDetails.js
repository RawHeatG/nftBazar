...
export function ProductDetails() {
    ...
    const CartButtons = () => {
        if(itemsInCart.find((item) => item.id === id)){
            return(
                <Link to="/cart"><button class="btn btn-primary interactions-button-cart" >Go To Cart</button></Link>
            )
        }
        return(
            <button class="btn btn-primary interactions-button-cart" onClick={() => dispatch({type: "MANIPULATE_CART", payload: {manipulation: "ADD_TO_CART", item: product}})} >Add To Cart</button>
        )
    }
        ...
        <>
            <div className="product-details">
                ...
                <div className="details">
                    <h1>{name}</h1>
                    <h2>By - {brand} {material}</h2>
                    <div>{ratings}⭐</div>
                    <h2>₹ {price}</h2>
                    {fastDelivery && <div class="delivery">Get it delivered in by tommorow!</div>}
